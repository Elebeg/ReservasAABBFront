...
function ReservationsPage() {
  ...
  // Carrega dados iniciais e também no refresh
  useEffect(() => {
  ...
  const handleUpdateReservation = async () => {
    ...
    try {
      ...
      setUserReservations(
        userReservations.map((res) => (res.id === editingReservationId ? response.data : res))
      );

      setAllReservations(
        allReservations.map((res) => (res.id === editingReservationId ? response.data : res))
      );
      ...
      <div className="reservations-header">
        ...
        <div className="reservation-quota-card">
          ...
          <div className="quota-progress-bar">
            <div className="quota-progress-fill" style={{ width: `${reservationProgress}%` }}></div>
          </div>
        ...
        <div className="reservations-form" ref={formRef}>
          ...
          <div className={`form-actions ${isEditing ? 'editing' : 'creating'}`}>
            <div className="form-actions-info">
              <span className="form-actions-title">{isEditing ? 'Editar reserva' : 'Nova reserva'}</span>
            ...
            <div className="form-actions-buttons">
              {isEditing ? (
                <>
                  <button className="btn btn-primary btn-lg" onClick={handleUpdateReservation} disabled={loading}>
                    ...
                    <div key={reservation.id} className={`reservation-card ${isPast ? 'past' : ''}`}>
                      <div className="reservation-info">
                        ...
                        <p className="reservation-detail">
                          ...
                          <span className="label">Data:</span> {format(reservationDate, 'dd/MM/yyyy', { locale: ptBR })}
                        </p>
                        <p className="reservation-detail">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                          ...
                          <span className="label">Horário:</span> {format(reservationDate, 'HH:mm', { locale: ptBR })}
                        </p>
                      </div>

                      {!isPast && (
                        <div className="reservation-actions">
                          <button className="btn-icon edit" onClick={() => handleEditReservation(reservation)} title="Editar reserva">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                              <path d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708l-3-3zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207l6.5-6.5zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.499.499 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11l.178-.178z"/>
                            </svg>
                          </button>
                          <button className="btn-icon delete" onClick={() => handleDeleteReservation(reservation.id)} title="Cancelar reserva">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                  ...
                  return (
                    <div
                      ...
                      className={`time-block ${
                        reservation ? (isUserReservation ? 'user-reserved' : 'reserved') : 'available'
                      }`}
                      title={
                        reservation
                    ...
                    >
                      ...
                      {reservation && (
                        <div className="reservation-tooltip">
                          <p>
                            <strong>Reservado por:</strong> {isUserReservation ? 'Você' : reservation.user?.name || 'outro usuário'}
                          </p>
                          <p>
